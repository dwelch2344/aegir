...
describe('GET /api/orgs/:orgId/members', () => {
  ...
  it('returns mapped member list', async () => {
    const rawMembers = [
      { id: 'u1', email: 'a@t.com', firstName: 'Alice', lastName: 'C', username: 'alice', enabled: true, emailVerified: true, extra: 'ignored' },
    ]
...
describe('POST /api/orgs/:orgId/members', () => {
  ...
  it('adds an existing user by userId', async () => {
    ...
    expect(mocks.kcAdmin).toHaveBeenCalledWith('/organizations/org-1/members', expect.objectContaining({
      method: 'POST',
      body: 'user-existing',
    }))
  })
  ...
  it('provisions a new user when email not found', async () => {
    ...
    expect(mocks.kcAdminRaw).toHaveBeenCalledWith('/users', expect.objectContaining({
      method: 'POST',
  ...
  it('throws 400 when neither userId nor email provided', async () => {
    ...
    const handler = await getHandler()
    await expect(handler(createMockEvent({ params: { orgId: 'org-1' } }))).rejects.toThrow('Either userId or email is required')
  })
})
