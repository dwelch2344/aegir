...
export async function registerDebeziumConnector(): Promise<void> {
  ...
  if (existing.ok) {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`Failed to update Debezium connector: ${resp.status} ${text}`)
    }
  ...
  if (!resp.ok) {
    const text = await resp.text()
    throw new Error(`Failed to create Debezium connector: ${resp.status} ${text}`)
  }
  console.log(`[cdc] created Debezium connector "${name}"`)
}

export async function getConnectorStatus(): Promise<any> {
  const resp = await fetch(`${DEBEZIUM_URL}/connectors/${CONNECTOR_CONFIG.name}/status`)
