...
import { registerSchema, encodeWithSchemaId } from "./schema-registry.js";
...
export async function registerSchemas(): Promise<void> {
  ...
  workflowSchemaId = await registerSchema(
    `${TOPICS.WORKFLOW_EVENTS}-value`,
    workflowAvroSchema,
  );
  taskSchemaId = await registerSchema(
    `${TOPICS.TASK_EVENTS}-value`,
    taskAvroSchema,
  );
}
    ...
    eachMessage: async ({ topic, message }) => {
      ...
      if (tableName === "workflow" || tableName === "workflow_instance") {
        ...
        if (event) {
          ...
          console.log(
            `[cdc] workflow ${event.event_type}: ${event.workflow_id} (${event.status})`,
          );
        }
      }
      ...
      if (
        ...
        if (event) {
          ...
          console.log(
            `[cdc] task ${event.event_type}: ${event.task_id} (${event.task_type} → ${event.status})`,
          );
        }
      }
    },
