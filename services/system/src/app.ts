...
export async function buildApp() {
  ...
  app.use(
    webPlugin({
      port: Number(getEnv('SYSTEM_PORT', '4002')),
      host: getEnv('SYSTEM_HOST', '0.0.0.0'),
    }),
  )
  ...
  app.use(
    pgPlugin({
      ...
      user: getEnv('PG_USER', 'system_svc'),
      password: getEnv('PG_PASSWORD', 'system_dev'),
      database: getEnv('PG_DATABASE', 'aegir'),
      searchPath: [getEnv('PG_SCHEMA', 'system')!],
  ...
  fastify.get('/health', async () => ({ status: 'ok', service: 'system' }))
