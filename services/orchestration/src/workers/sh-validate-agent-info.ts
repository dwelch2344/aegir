...
export async function handleShValidateAgentInfo(task: any): Promise<TaskResult> {
  const {
    ...
    agentNpn, residentState, requestedStates,
    hasC4CoCertification, hasCoLicense,
  ...
  if (!agentEmail || typeof agentEmail !== 'string' || !agentEmail.includes('@'))
    errors.push('Valid email is required')
  if (!agentPhone || typeof agentPhone !== 'string')
    errors.push('Phone number is required')
  if (!agentNpn || typeof agentNpn !== 'string')
    errors.push('NPN (National Producer Number) is required')
  if (!residentState || typeof residentState !== 'string')
  ...
  if (errors.length === 0) {
    ...
    if (!isResident) {
      ...
      if (wantsCo && hasC4CoCertification && hasCoLicense && onlyCo) {
        ...
        errors.push(`Agent must be a resident of UT, ID, CO, or NV to appoint with Select Health. Resident state "${residentState}" is not eligible.`)
      }
    } else {
