...
export async function buildApp() {
  ...
  app.use(
    webPlugin({
      port: Number(getEnv('PRACTICES_PORT', '4005')),
      host: getEnv('PRACTICES_HOST', '0.0.0.0'),
    }),
  )
  ...
  app.use(
    pgPlugin({
      ...
      searchPath: [getEnv('PG_SCHEMA', 'practices')],
  ...
  fastify.get('/health', async () => ({ status: 'ok', service: 'practices' }))
